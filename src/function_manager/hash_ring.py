...
class HashRing:
    ...
    def can_hold_key(self, container, cache_capacity, key, cached_keys):
        # 加了当前的key之后，容器中的cached_key数量仍没有达到替换条件
        cur_num_of_cached_keys = len(set(cached_keys) | {key})
        print(
            f"cached_keys/cache_capacity: {cur_num_of_cached_keys}/{cache_capacity}")
        return cur_num_of_cached_keys <= cache_capacity
    ...
    def get_container(self, current_key, container_cached_key, cur_container_pool, cache_capacity):
        ...
        if container_cached_key:
            ...
            print(
                f"Seeking the container with {len(container_cached_key)} of cached_key ({container.container.name})")
            return container
        return None
