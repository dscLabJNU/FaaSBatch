...
function fetch_csvs() {
    ...
    transfered_csv=$(printf "${csv_prefix}" "${strategy}_${azure_type}_${dispatch_interval}")
...
function fetch_provisioned_containers() {
    ...
    csv_file_name="num_containers_${strategy}_${dispatch_interval}.csv"

    # $path_to_save_csvs/

    colums="strategy,dispatch_interval,num_containers"
    rm -rf $csv_file_name
    ...
    echo "$strategy,$dispatch_interval,$num_containers" >>$csv_file_name
